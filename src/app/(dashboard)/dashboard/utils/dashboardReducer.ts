  ...
  if (behavior === "exclusive") {
    ...
    if (inCurrentFolder) {
      ...
      const index = parentChildren.findIndex((child) =>
        checkItemID(child, getItemID(item))
      );
...
const undisplayOne = (
  ...
  dashboard.displayedItems = dashboard.displayedItems.filter(
    (displayedItem) => !compareItems(displayedItem, item)
  );
...
const undoCutOne = (dashboard: DashboardView, item: DashboardItem) => {
  ...
  dashboard.clipboard.cut = itemsCut.filter((itemCut) => {
...
const openFolder = (
  ...
  folderData: DashboardView["dataOfCurrentFolder"]
) => {
  dashboard.currentFolder = folder;
  dashboard.selected = [];
